...
import { checkMalSequentialBaseLength } from "./checker";
...
function readForm(reader: Reader): MalType {
  switch (reader.peek()) {
    ...
    case "'":
      return readSymbol(reader, Symbols.Quote);
    case "`":
      return readSymbol(reader, Symbols.Quasiquote);
    case "~":
      return readSymbol(reader, Symbols.Unquote);
    case "~@":
      return readSymbol(reader, Symbols.SpliceUnquote);
    case "@":
      return readSymbol(reader, Symbols.Deref);
    default:
      return readAtom(reader);
  }
}

function read<T extends MalType>(
  reader: Reader,
  first: string,
  end: string
): Array<MalType> {
  ...
  if (firstToken !== first)
    throw new MalReadError(`${firstToken} is not ${first}`);
...
function readHashMap(reader: Reader): MalHashMap {
  ...
  checkMalSequentialBaseLength(tokenList, 2);
