...
ReplEnv.set(
  MalSymbol.get("//"),
  new MalNativeFunction(
    (x: any, y: any) => new MalNumber(Math.floor(x.value / y.value))
  )
);
...
function EVAL(ast: MalType, env: MalEnv): any {
  if (!(ast instanceof MalList)) {
    ...
    if (!(symbol instanceof MalSymbol))
      throw `Unexcepted Token Type: {${symbol}}, except MalSymbol`;
  ...
  function DEF(env: MalEnv, key: MalType, value: MalType): MalType {
    if (!(key instanceof MalSymbol))
      throw `Unexcepted Token Type: {${key}}, except MalSymbol`;
    return env.set(key, value);
  }

  function LET(env: MalEnv, bindings: MalType, ast: MalType): MalType {
    const newEnv = new MalEnv(env);
    if (!(bindings instanceof MalList))
      throw `Unexcepted Token Type: {${bindings}}, except MalList`;
