...
export class MalUnexpectedTokenType extends MalError {
  ...
  constructor(token: MalType, ...excepted: Array<any>) {
    ...
    this.message = `Unexcepted token type: ${
      this.token
    }, excepted: ${this.excepted.map(item => item.name).join(", ")}`;
  }
}
...
export class MalInvalidRestParameter extends MalError {
  ...
  constructor(instance: MalType) {
    ...
    this.message = `Invalid rest parameter: ${
      this.instance
    } should be set as the last one`;
  }
}
...
export class MalUnexpectedLength extends MalError {
  ...
  constructor(instance: MalVector, base: number) {
    ...
    this.base = base;
    this.instance = instance;
    this.message = `Unexpected length: ${
      this.instance.length
    }, excepted multiple of ${this.base} - ${this.instance}`;
  }
}
...
export class MalParametersError extends MalError {
  ...
  constructor(symbol: MalSymbol, called: number, excepted: number) {
    ...
    this.called = called;
    this.excepted = excepted;
    this.message = `${this.symbol} needs ${this.excepted} parameters, called ${
      this.called
    }`;
  }
}
...
export class MalMultipleParametersError extends MalParametersError {
  constructor(symbol: MalSymbol, called: number, excepted: number) {
    ...
    this.message = `${this.symbol} needs at least ${
      this.excepted
    } parameters, called ${this.called}`;
  }
}
...
export class MalIndexOutOfRangeError extends MalError {
  ...
  constructor(list: MalVector, index: MalNumber) {
    ...
    this.list = list;
    this.index = index;
    this.message = `Index ${this.index} is out of range ${
      this.list.length
    } of ${this.list}`;
  }
}
