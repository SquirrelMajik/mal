...
export class MalVector extends MalType {
  ...
  map(
    callback: (item: MalType, index: number, array: Array<MalType>) => any
  ): Array<any> {
...
export class MalHashMap extends MalType {
  ...
  update(mapValues: Array<Array<MalType>>): this {
    mapValues.forEach(mapValue => {
      const [key, value] = mapValue;
      this.set(key, value);
    });
  ...
  copy(): MalHashMap {
    let newHashMap = new MalHashMap([]);
    newHashMap.keywordMap = new Map(this.keywordMap);
    newHashMap.stringMap = new Map(this.stringMap);
    return newHashMap;
  }
  ...
  values(): Array<MalType> {
    return [...this.keywordMap.values(), ...this.stringMap.values()];
  }
...
export class MalError extends MalType {
  message: string = "MalBaseError";
  callStack: Array<MalType> = [];
  callStackPrefix: string = "    -> ";

  pushAst(ast: MalType) {
    this.callStack.push(ast);
  }
  ...
  callStackString(): string {
    return this.callStack
      .slice(0, 10)
      .map(ast => `${this.callStackPrefix}${printString(ast)}`)
      .join("\n");
  }
}
...
export const enum Symbols {
  ...
  HashMap = "hash-map",
  isHashMap = "map?",
  HashMapAssociate = "assoc",
  HashMapDissociate = "dissoc",
  HashMapGet = "get",
  HashMapContains = "contains?",
  HashMapKeys = "keys",
  HashMapValues = "vals",
